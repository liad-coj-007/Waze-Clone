...
KDNode* KDTree::find_closest(KDNode*& target,
    ...
    if(getDistance(target,node) < getDistance(target,best)){
        best = node;
    }
    ...
    if(delta < getDistance(best,target)){
...
double KDTree::getDistance( KDNode*& p1, KDNode*& p2)const{
    double dx = p1->lat - p2->lat;
    double dy = p1->lon - p2->lon;
